...
import AppListActs from "../appList/ListActs";
...
const HomeActs = (props) => {
    const [len, setLen] = useState("?len=10")
    const [val, setVal] = useState("")

    var API_URL = API_URL_I + "acts/" + len
    
    if (props.search !== undefined) {
        API_URL = API_URL_I + "acts_gen" + props.search
    } else if (val) {
        API_URL = API_URL_I + "acts_gen" + "?search=" + val
    }
        ...
        <Container fluid>
            <Row xs="12">
                <Col xs={{
                    ...
                    <AppModalActs create={true} resetState={refetchData} newItem={true} />
                </Col>
                <Col xs={{
                    size: 'auto',
                    order: 2,
            ...
            <Row xs="2">
                <Col xs="12" className="list-content">
                    <Card shadow={false} >
                        <div>
                            <AppListActs data={dataSorted()} resetState={refetchData} newItem={false} />
                        </div>
                    </Card>
                </Col>
            </Row>
