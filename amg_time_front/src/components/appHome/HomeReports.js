...
import AppListReports from "../appList/ListReports";
...
const HomeReports = (props) => {
    const [len, setLen] = useState("?len=10")

    const API_URL = API_URL_I + "reports/" + len
    ...
    const dataSorted = () => {
        return data ? data.sort((a, b) => b.pk - a.pk) : data
    }
        ...
        <Container fluid>
            <Row xs="12">
                <Col xs={{
                    ...
                    <AppModalReports create={true} resetState={refetchData} newItem={true} />
                </Col>
                <Col xs={{
                    size: 'auto',
                    order: 2,
            ...
            <Row xs="2">
                <Col xs="12" className="list-content">
                    <Card shadow={false} >
                        <div>
                            <AppListReports data={dataSorted()} resetState={refetchData} newItem={false} />
                        </div>
                    </Card>
                </Col>
            </Row>
